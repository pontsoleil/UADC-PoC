...
HMD = ROOT/'models/gl-cor/accounting-entries/hmd/XBRL_GL_Next_HMD_AccountingEntries_for_taxonomy.csv'
...
def execute(label, command):
    p = subprocess.run([sys.executable, str(ROOT/'tools/binding/flat_csv.py'), *map(str, command)], capture_output=True, text=True)
...
for basis in ['anonymous_v17']:
    ...
    execute(basis+' EPSON xBRL-CSV generation', ['to-structured', target, '-o', oim_folder/'accounting_entries.csv', *common,
        ...
        '--taxonomy-entrypoint',ROOT/'taxonomy/accounting-entries/oim/cor_accountingEntries/cor-all-oim-2026-12-31.xsd',
