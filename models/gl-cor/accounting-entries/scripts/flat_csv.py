#!/usr/bin/env python3
"""Canonical 17-column Flat CSV converter.
...
COLUMN_RE = re.compile(r"^Column([1-9][0-9]*)$")
...
TRANSFORMATION_NAMES = {"", "jp_era_date_to_iso"}
...
def _parse_group_spec(row: BindingRow) -> GroupSpec:
    ...
    if row.values["occurrence_mode"] == "keyed_rows":
        if not isinstance(config, list) or not config:
            raise ConversionError(
                "GROUP_KEY_INVALID", "keyed_rows group_key must be a non-empty ColumnN array"
            )
        columns = tuple(str(item) for item in config)
        if any(_column_number(column) is None for column in columns):
            raise ConversionError(
                "GROUP_KEY_INVALID", "keyed_rows group_key must contain only ColumnN names"
            )
    ...
    if _column_number(marker_column) is None:
        raise ConversionError("GROUP_KEY_INVALID", "marker_column must be a ColumnN name")
    ...
    try:
        with path.open(newline="", encoding=encoding) as stream:
            ...
            if header != CANONICAL_FIELDS:
                if len(header) == 13:
                    raise ConversionError(
                        "UNSUPPORTED_LEGACY_BINDING", "unsupported legacy Binding; exact 17-column header required"
                    )
                raise ConversionError(
                    "BINDING_HEADER_INVALID", "Binding header must exactly match the canonical 17 columns"
                )
            rows: list[BindingRow] = []
            for order, values in enumerate(reader):
                if len(values) != len(CANONICAL_FIELDS):
                    raise ConversionError(
                        "BINDING_ROW_INVALID", "a Binding row does not have exactly 17 columns"
                    )
                rows.append(
                    BindingRow(
                        order=order,
                        values={name: value.strip() for name, value in zip(CANONICAL_FIELDS, values)},
                    )
                )
...
def validate_definition(
    ...
    for row in rows:
        ...
        if column:
            if _column_number(column) is None:
                raise ConversionError(
                    "PHYSICAL_COLUMN_INVALID", "physical columns must use positive ColumnN names"
                )
    ...
    drivers = [row for row in rows if row.values["row_role"] == "driver"]
    if len(drivers) != 1:
    ...
    header_groups = [
        row for row in rows if row.values["occurrence_mode"] in {"keyed_rows", "marker_rows"}
    ]
...
def _definition(options: ConversionOptions) -> Definition:
    ...
    return validate_definition(rows, hmd_sequences, hmd_rows)
...
def _read_flat_rows(path: Path, encoding: str, width: int) -> list[list[str]]:
    try:
        with path.open(newline="", encoding=encoding) as stream:
...
def _transform_forward(name: str, value: str) -> str:
...
def _transform_reverse(name: str, value: str) -> str:
    if not value or not name:
        return value
    if name == "jp_era_date_to_iso":
        try:
...
def _value_for(row: BindingRow, source: Sequence[str]) -> str:
    column = row.values["column"]
    if column:
    ...
    return _transform_forward(row.values["transformation"], value)
...
def _active_rows(
    rows: Sequence[BindingRow], source: Sequence[str], driver: BindingRow
) -> list[BindingRow]:
    suppressed: list[str] = []
    for row in rows:
        ...
        if physical and not any(_value_for(candidate, source) for candidate in physical):
...
def _oim_records(
    ...
    for source in records:
        ...
        namespaces[module] = (
            f"https://www.xbrl.or.jp/taxonomy/xbrl-gl-next/{module}"
        )
...
def _write_oim_metadata(
    ...
    namespace_map = {
        **dict(sorted(namespaces.items())),
        "plt": (
            "https://www.xbrl.or.jp/taxonomy/xbrl-gl-next/plt"
        ),
...
def convert_to_structured(options: ConversionOptions) -> ConversionSummary:
    ...
    source_rows = _read_flat_rows(options.input_path, options.input_encoding, width)
    ...
    for row in sorted(root_rows, key=lambda item: (item.sequence, item.order)):
        value = _value_for(row, empty_source)
    ...
    for entry_key, grouped_rows in groups:
        ...
        for row in sorted(header_rows, key=lambda item: (item.sequence, item.order)):
            value = _value_for(row, selected_header_source)
            _require_value(row, value)
        ...
        for source_index, source in grouped_rows:
            for variant in definition.variants:
                ...
                if presence and not any(_value_for(row, source) for row in presence):
                    continue
                active = _active_rows(
                    [definition.driver, *selected, *shared_rows], source, definition.driver
                )
                for row in sorted(active, key=lambda item: (item.sequence, item.order)):
                    value = _value_for(row, source)
                    _require_value(row, value)
                    records.append(
...
def _validated_structured_rows(
    ...
    for row in rows:
        binding_row = by_path.get(row["binding_path"])
        if binding_row is None:
            raise ConversionError(
                "STRUCTURED_BINDING_MISMATCH", "Structured binding_path is absent from Binding"
            )
        if row["semantic_path"] != binding_row.neutral_path:
            raise ConversionError(
...
def _write_flat(path: Path, rows: Iterable[Sequence[str]], encoding: str) -> None:
    try:
        with path.open("w", newline="", encoding=encoding) as stream:
...
def convert_to_flat(options: ConversionOptions) -> ConversionSummary:
    ...
    ordered_detail_groups = sorted(details.items(), key=lambda item: item[0][1])
    for (entry_key, source_row), rows in ordered_detail_groups:
        ...
        for structured in ordered:
            ...
            value = _transform_reverse(
                binding_row.values["transformation"], structured["value"]
            )
    ...
    _write_flat(options.output_path, output_rows, options.output_encoding)
    summary = ConversionSummary(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Canonical 17-column Flat CSV converter")
    ...
    add_common(reverse)
    reverse.add_argument("--output-encoding", default="utf-8-sig")
