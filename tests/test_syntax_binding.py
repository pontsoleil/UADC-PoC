...
from phase1_helpers import PHASE1_BINDING, convert_phase1
...
def main() -> int:
    ...
    out_csv = convert_phase1(ROOT / "samples" / "input" / f"{SOURCE_XML_NAME}.xml", out_dir)
    assert out_csv.name == PHASE1_CSV_NAME
    assert PHASE1_BINDING.name == "EN16931_UBL_Invoice_Syntax_Binding.csv"
    ...
    with PHASE1_BINDING.open(newline="", encoding="utf-8-sig") as f:
        binding_fields = csv.DictReader(f).fieldnames or []
    assert "structured_csv_column" in binding_fields
    assert "structured_csv_level" in binding_fields
